...
# Holding the pan above the serving plate, tip it so the omelette rolls off, folding itself onto the plate. The two edges will be tucked underneath.", :date=>Time.now},
...
# Add potatoes, chicken, coconut milk, and 1 1/2 cups water and bring to a boil. Reduce heat to a simmer and cook, stirring occasionally, until chicken is cooked through and potatoes are tender, about 20 minutes. Divide curry among bowls and top with herbs.", :date=> Time.now}
# ].each do |recipe|
# 	rcp = Recipe.new
...
# 	rcp.save
# end
# [{:title=> "This is cool", :comment=>"The Food is really delicious.",:rating=> 4, :date=>Time.now, :recipe_id=>1},
# {:title=> "This is bad!", :comment=> "This isn't a good recipe.", :rating=> 1, :date=> Time.now, :recipe_id=>1},
# {:title=> "I love it!", :comment=> "OMG, that is purely delicious", :rating=> 5, :date=> Time.now, :recipe_id=>1}
# ].each do |review|
# 	rv = Review.new
